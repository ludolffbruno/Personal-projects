...
```powershell
python -m pip install pyinstaller requests pymupdf
```
...
```powershell
python -m PyInstaller --noconsole --onefile --clean --collect-all requests --collect-all pymupdf --add-data "graph_email_monitor.py;." --name "automacao-email-alex" nf_automation_gui.py
```
...
> [!TIP]
> **Performance**: O sistema processa até **50 e-mails por vez**, garantindo agilidade mesmo em dias com muito volume de notas.

> [!IMPORTANT]
> **Dependências Externas**: 
> Lembre-se que o computador que for rodar o executável ainda precisa ter o **pdftotext** instalado no Windows (PATH) ou configurado localmente, conforme as instruções do guia de instalação.
